...
const CONTENT_MANAGEMENT_TOKEN =
  "CFPAT-nNKy6WQGgarKDwSc0xxORKz_wxAqhYR2mQW_XfkEcmg";
  ...
  async createEntry(contentTypeId, data) {
    try {
      const entry = await this.environment.then((e) =>
        e.createEntry(contentTypeId, { fields: data })
      );
      return entry;
    } catch (e) {
      throw new Error(`error creating entry of ${contentTypeId}: ${e}`);
    }
  },
